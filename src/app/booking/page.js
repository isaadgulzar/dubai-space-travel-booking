...
export default function BookingPage() {
	...
	const [selectedDestination, setSelectedDestination] = useState(
		initialDestinationId ? parseInt(initialDestinationId) : null
	)
	const [selectedPackage, setSelectedPackage] = useState(
		initialPackageId ? parseInt(initialPackageId) : null
	)
	...
	const [selectedAccommodation, setSelectedAccommodation] = useState(
		initialAccommodationId ? parseInt(initialAccommodationId) : null
	)
	...
	useEffect(() => {
		...
		if (selectedPackage) {
			...
			if (
				packageData &&
				(!selectedDestination ||
					selectedDestination !== packageData.destinationId)
			) {
	...
	useEffect(() => {
		if (selectedAccommodation) {
			const accommData = accommodations.find(
	...
	useEffect(() => {
		if (bookingMode === 'package' && currentPackageData) {
			...
			const baseTravelCost =
				currentDestinationData.price[selectedSeatClass] || 0
			const newTravelCost = baseTravelCost * passengers
			setTravelCost(newTravelCost)

			// Calculate accommodation cost
			if (currentAccommodationData) {
				const newAccommodationCost =
					currentAccommodationData.pricePerNight *
					accommodationNights *
					Math.ceil(passengers / 2)
								...
								{currentDestinationData && (
									<div className="mb-6">
										...
										<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
											{Object.entries(currentDestinationData.price).map(
												([className, price]) => {
													...
													return (
														...
														>
															<div className="p-4">
																...
																<p className="text-indigo-300 font-bold text-xl mb-2">
																	${price.toLocaleString()}
																</p>
																<div className="text-gray-400 text-sm">
																	{className === 'economy' && (
																		<p>
																			Standard space travel experience with
																			essential amenities.
																		</p>
																	)}
																	{className === 'business' && (
																		<p>
																			Enhanced comfort with premium food and
																			larger personal space.
																		</p>
																	)}
																	{className === 'luxury' && (
																		<p>
																			Ultimate space travel with private cabins
									...
									<div className="mb-6">
										<div className="flex justify-between items-center mb-4">
											...
											<div className="flex items-center">
												<label className="block text-gray-300 mr-2">
													Nights:
												</label>
												<select
													...
													onChange={(e) =>
														setAccommodationNights(parseInt(e.target.value))
													}
												>
												...
												>
													<div className="p-4">
														...
														<p className="text-indigo-300 font-bold mb-2">
															${accomm.pricePerNight.toLocaleString()}{' '}
